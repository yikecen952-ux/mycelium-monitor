"""
sensor_env.py · Environment humidity collection
ESP32-B + BME280-B, COM4, placed beside specimen to measure ambient air

Usage: python sensor_env.py
Stop:  Ctrl+C
"""
...
def init_csv():
    ...
    if not p.exists():
        with open(CSV_FILE, "w", newline="", encoding="utf-8-sig") as f:
            ...
            writer.writerow([
                "datetime", "sample_id",
                "time_ms", "temp_c", "env_humidity", "pressure_hpa"
            ])
    return open(CSV_FILE, "a", newline="", encoding="utf-8-sig")


def send_env(temp, env_hum, pressure):
    if not SEND_TO_DB:
        return
    try:
        headers = {"Authorization": f"Bearer {AUTH_TOKEN}"} if AUTH_TOKEN else {}
        res = requests.post(f"{API}/environment", json={
            "sample_id":    SAMPLE_ID,
            "temp_c":       temp,
...
print("=" * 50)
print(f"  Environment humidity  |  Sample: {SAMPLE_ID}")
...
try:
    while True:
        ...
        try:
            time_ms  = parts[0]
            temp     = float(parts[1])
            env_hum  = float(parts[2])
            pressure = float(parts[3])
        except ValueError:
            continue

        # Save to CSV (always)
        writer.writerow([now, SAMPLE_ID, time_ms, temp, env_hum, pressure])
        f.flush()

        print(f"[{now}] Temp {temp}°C | Env humidity {env_hum}% | Pressure {pressure}hPa")

        send_env(temp, env_hum, pressure)
